...
span.stars span {
    ...
    position: relative;
    margin: 0;
    background: url('src/stars_big.png') 0 -32px repeat-x;
    width: 120px;
    height: 24px;
}
span.stars span {
...
<body>
	...
	<section id="blog">
	<c:forEach items="${page.content}" var="rcp">
		<div class="blog-box">
			<div class="blog-img">
				<a href="${contextRoot}/recipe/${rcp.rid}"><img src="${contextRoot}/${rcp.img}"></a>
			</div>
			<div class="blog-details">
				<h4>${rcp.title}</h4>
				<span class="stars">${rcp.rates}</span>
				<br>
				<p>${rcp.rtp.rtype} / ${rcp.rtime} 分鐘 / ${rcp.serve} 人份</p>
				<br> <a href="${contextRoot}/recipe/${rcp.rid}">繼續閱讀</a>
			</div>
		</div>
	</c:forEach>
	</section>
		...
		<c:forEach var="pageNumber" begin="1" end="${page.totalPages}">
			<c:choose>
				<c:when test="${page.number+1 != pageNumber}">
					<a
						href="${contextRoot}/recipe?p=${pageNumber}">${pageNumber}</a>
				</c:when>
				<c:otherwise>
					<a>${pageNumber}</a>
				</c:otherwise>
